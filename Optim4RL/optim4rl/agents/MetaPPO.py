...
class MetaPPO(PPO):
  ...
  def train(self):
    ...
    # L2 regularization weight for agent parameters
    l2_weight = self.cfg['agent'].get('l2_weight', 0.0)
    # Others
    ...
    # Add L2 regularization to agent parameters
    def agent_loss_fn_with_l2(params, normalizer_params, data, rng):
      """Compute PPO loss with L2 regularization on agent parameters."""
    ...
    # Use L2-regularized loss for both agent and meta training
    agent_loss_fn = agent_loss_fn_with_l2
    meta_loss_fn = agent_loss_fn_with_l2
    ...
    def meta_training_step(meta_param, meta_optim_state, training_state, env_state, key):
      ...
      # Update meta_param (pass meta_param for transformations like weight_decay)
      meta_param_update, meta_optim_state = meta_optim.update(meta_grad, meta_optim_state, meta_param)
