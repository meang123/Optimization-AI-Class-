...
from utils.helper import jitted_split, save_model_param
...
class PPO(object):
  ...
  def __init__(self, cfg):
    ...
    # Apply non-stationary wrapper if configured
    if cfg['env'].get('non_stationary', False):
      task_type = cfg['env'].get('task_type', self.env_name)
      change_mode = cfg['env'].get('change_mode', 'continuous')
      self.env = NonStationaryDynamicsWrapper(self.env, task_type=task_type, change_mode=change_mode)
      self.logger.info(f'Non-stationary dynamics wrapper applied: task_type={task_type}, change_mode={change_mode}')
  ...
  def train(self):
    ...
    l2_weight = self.cfg['agent'].get('l2_weight', 0.0)  # L2 regularization weight for agent parameters
    ...
    # Policy saving
    save_policy_interval = self.cfg.get('save_policy_interval', 0)  # 0 = don't save, >0 = save every N iterations
    ...
    def loss_fn_with_l2(params, normalizer_params, data, rng):
      """PPO loss function with L2 regularization on agent parameters"""
      # Compute standard PPO loss
      loss, aux = ppo_losses.compute_ppo_loss(
        params=params,
      ...
      # Add L2 regularization penalty if l2_weight > 0
      if l2_weight > 0:
        l2_penalty = sum(
          jnp.sum(jnp.square(p))
          for p in jax.tree_util.tree_leaves(params)
        )
        total_loss = loss + l2_weight * l2_penalty
      else:
        total_loss = loss

      return total_loss, aux

    loss_fn = loss_fn_with_l2
    ...
    for i in range(1, num_evals+1):
      ...
      if self.process_id == 0:
        ...
        if save_policy_interval > 0 and (i % save_policy_interval == 0 or i == num_evals):
          policy_save_path = self.cfg['logs_dir'] + f'agent_param_iter{i}.pickle'
          inference_param_save_path = self.cfg['logs_dir'] + f'inference_param_iter{i}.pickle'
          save_model_param(training_state.agent_param, policy_save_path)
          # Also save inference params (normalizer + policy) for direct inference use
          inference_params = (training_state.normalizer_param, training_state.agent_param.policy)
          save_model_param(inference_params, inference_param_save_path)
          self.logger.info(f'Saved policy at iteration {i}: {policy_save_path}')
