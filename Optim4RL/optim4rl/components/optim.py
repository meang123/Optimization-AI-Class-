...
def set_optim(optim_name, original_optim_cfg, key):
  ...
  if optim_name in ['LinearOptim', 'L2LGD2'] or 'Optim4RL' in optim_name:
    ...
    del optim_cfg['grad_clip'], optim_cfg['grad_norm'], optim_cfg['weight_decay']
    ...
    if grad_clip > 0:
      optim_components.append(optax.clip(grad_clip))
    elif grad_norm > 0:
      optim_components.append(optax.clip_by_global_norm(grad_norm))

    # Add weight decay (L2 regularization) if specified
    if weight_decay > 0:
      optim_components.append(optax.add_decayed_weights(weight_decay))

    # Add the base optimizer
    optim_components.append(getattr(optax, optim_name.lower())(**optim_cfg))

    # Chain all components together
    if len(optim_components) > 1:
      optim = optax.chain(*optim_components)
    else:
      optim = optim_components[0]
  return optim
